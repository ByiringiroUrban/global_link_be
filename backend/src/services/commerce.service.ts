...
export async function getAdminStats() {
  const [
    ...
    prisma.user.count({ where: { role: Role.SUPPLIER } }),
    ...
    prisma.$queryRaw<
      Array<{ id: string; quantity: number; lowStockThreshold: number; productId: string }>
    >`
...
export async function getSupplierInventory(supplierUserId: string) {
  const supplier = await prisma.supplierProfile.findUnique({
    where: { userId: supplierUserId },
  });

  if (!supplier) {
    throw ApiError.notFound('Supplier profile not found');
  }
...
export async function getUserOrders(userId: string, page: number, limit: number, skip: number) {
...
export async function listProducts(
  ...
  limit: number,
  skip: number,
  filters: { category?: string; search?: string; supplierId?: string }
) {
...
export async function createProduct(supplierUserId: string, input: CreateProductInput) {
  const supplier = await prisma.supplierProfile.findUnique({
    where: { userId: supplierUserId },
  });
...
export async function updateInventory(
  ...
  productId: string,
  data: { quantity?: number; warehouseLocation?: string; lowStockThreshold?: number }
) {
  const supplier = await prisma.supplierProfile.findUnique({
    where: { userId: supplierUserId },
  });
  ...
  if (!product) {
    throw ApiError.notFound('Product not found or not owned by supplier');
  }
...
export async function createOrder(userId: string, shippingAddress: string, notes?: string) {
  const cart = await prisma.cart.findUnique({
    where: { userId },
    include: {
      items: {
  ...
  for (const item of cart.items) {
    const available =
      (item.product.inventory?.quantity || 0) - (item.product.inventory?.reservedQuantity || 0);
